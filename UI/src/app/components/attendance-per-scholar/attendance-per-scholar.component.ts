...
export class AttendancePerScholarComponent implements HasUnsavedChanges {
  ...
  // The scholar and all their attendance. The attendance list
  // is only shown (and so only saveable) once both have loaded: Save replaces
  // the scholar's whole list, so saving over a failed load would wipe the
  // attendance that couldn't be read. hasValue() guards the reads: value()
  // throws while the resource is in error.
  private readonly data = rxResource({
    params: () => this.scholarId(),
    // The scholar first: no attendance request for a scholar that doesn't exist.
  ...
  // The school's name, for the printable invoice header.
  readonly schoolName = computed(() => this.school()?.name ?? '');

  // 'YYYY-MM', the value format of <input type="month">. A one-field signal
  // form binds the picker; selectedMonth is its value. It defaults, once the
  // attendance loads, to the most recent month that has data.
...
// The most recent month that already has data, or DEFAULT_MONTH if this
// scholar has no attendance yet.
