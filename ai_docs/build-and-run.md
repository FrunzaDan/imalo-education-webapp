...
- `API/ImaloEducationApi.Tests/` — xUnit project for the API, see [[api]] Gotchas.
...
2. `dotnet test` the API's xUnit project (`API/ImaloEducationApi.Tests`, Debug).
...
4. **API.** Starts in the background: `ASPNETCORE_ENVIRONMENT=Development dotnet run --project API/ImaloEducationApi --launch-profile ImaloEducationApi`, plain HTTP at `http://localhost:5244` (read from `Properties/launchSettings.json`'s `applicationUrl`, falling back to that default). Waits for `/swagger/index.html` to respond (up to 60s). Logs to `.run/api.log`.
